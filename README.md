...
- Run `uv run cli/evaluation_cli.py`
  - Optional `--limit` flag determines number of results to evaluate
    ...
    - Query: dinosaur park
        ...
        - Recall@5: 1.0000
        - F1 Score: 0.3333
        - Retrieved: ['Jurassic Park', 'Lost River', 'Carnosaur', 'The Crater Lake Monster', 'Ice Age: Dawn of the Dinosaurs']
        - Relevant: ['Jurassic Park']
    ```

- Run `uv run cli/hybrid_search_cli.py rrf-search "<query>" --evaluate`.
  - Optional `--limit` flag returns custom number of search results.
  - Optional `--k` flag controls weight given to lower/higher scores.
  (low k means low scores given less weight)
  - eg.`uv run cli/hybrid_search_cli.py rrf-search "scary robot movie" --evaluate`

    ```
    1. Armitage: Dual Matrix: 1/3
    2. Heartbeeps: 1/3
    3. Alvin and the Chipmunks Meet the Wolfman: 0/3
    4. Scary Movie 4: 2/3
    5. Miami Vice: 0/3
    ```
